...
static void onRecv(const esp_now_recv_info_t* info, const uint8_t* data, int len) {
  ...
  // Debug: Log packet reception rate
  static unsigned long lastDebugLog = 0;
  static uint32_t packetCount = 0;
  packetCount++;
  unsigned long now = millis();
  if (now - lastDebugLog >= 5000) {
    float packetsPerSec = packetCount / 5.0f;
    Serial.printf("[ESP-NOW] Receiving %.1f packets/sec (L=%d R=%d)\n", 
      packetsPerSec, p->leftSpeed, p->rightSpeed);
    packetCount = 0;
    lastDebugLog = now;
  }
  ...
  /* When dock homing or autonomous docking is active, don't apply drive commands — main loop uses dock motor output */
  if (dockHomingIsActive() || autonomousDockingIsActive()) {
    lastCommandMillis = millis();
    return;
  }
  ...
  // Rate-limit display updates to prevent visual jumpiness (update every 100ms = 10Hz)
  static unsigned long lastDisplayUpdate = 0;
  unsigned long now = millis();
  if (now - lastDisplayUpdate >= 100) {
    // Only show IDLE if both speeds are actually zero
    bool isMoving = (abs(left) > 10) || (abs(right) > 10);  // 10/255 ~= 4% threshold
    displaySetCommand(isMoving ? CMD_DRIVE : CMD_IDLE);
