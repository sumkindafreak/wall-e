...
// Target heading for orientation control
static float s_targetHeading = -1.0f;  // -1 = not set
...
void compassSetTargetHeading(float degrees) {
  while (degrees < 0) degrees += 360.0f;
  while (degrees >= 360.0f) degrees -= 360.0f;
  s_targetHeading = degrees;
}

float compassGetTargetHeading() {
  return s_targetHeading;
}

float compassGetHeadingError() {
  if (s_targetHeading < 0) return 0.0f;
  float error = s_targetHeading - s_headingDeg;
