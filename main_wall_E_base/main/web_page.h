  ...
  <style>
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    #btn-clear-creds {
      ...
      color: var(--txt-dim); font-size: 0.62rem; font-weight: 600;
      letter-spacing: 0.06em; text-transform: uppercase; cursor: pointer;
      transition: border-color 0.15s, color 0.15s;
    }
    #btn-clear-creds:hover { border-color: var(--warn); color: var(--warn); }
    #btn-clear-creds:disabled { opacity: 0.3; cursor: default; }
    ...
    #network-list { display: flex; flex-direction: column; gap: 4px; max-height: 200px; overflow-y: auto; }
    .net-item {
    ...
    .net-name { flex: 1; font-size: 0.85rem; font-weight: 500; color: var(--txt); overflow: hidden; text-overflow: ellipsis; white-space: nowrap; min-width: 0; }
    .net-meta { display: flex; align-items: center; gap: 8px; flex-shrink: 0; }
    .net-rssi { font-size: 0.7rem; color: var(--txt-dim); font-family: monospace; }
    ...
    .scan-hint { font-size: 0.68rem; color: var(--txt-dim); margin-top: 6px; }
...
<div id="app">
  ...
  <div id="tabbar">
    ...
    <div class="tab" data-tab="autonomy" onclick="switchTab('autonomy')">
      <svg viewBox="0 0 24 24" fill="none" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 2a10 10 0 0 1 10 10c0 5.5-4.5 10-10 10S2 17.5 2 12 6.5 2 12 2z"/><circle cx="12" cy="12" r="2"/><path d="M12 6v2M12 16v2M6 12h2M16 12h2"/></svg>
      Autonomy
    </div>
  ...
  <div id="pages">
    ...
    <div class="page" id="page-connections">
      ...
      <div class="card">
        <div class="card-header">
          ...
          <div style="display:flex; gap:8px; align-items:center">
            <button id="btn-clear-creds" onclick="doClearCredentials()" title="Forget saved network">Clear saved</button>
        ...
        <div class="card-body">
          ...
          <div class="status-row">
            ...
            <span class="value" id="st-ap-clients">0</span>
          </div>
          <div class="status-row">
        ...
        <div class="card-body">
          ...
          <div class="form-group">
            ...
            <div class="pass-wrap">
              ...
              <button type="button" class="pass-toggle" onclick="togglePassVisible()" title="Show password" aria-label="Toggle password visibility">
                <svg id="pass-eye" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/><circle cx="12" cy="12" r="3"/></svg>
                <svg id="pass-eye-off" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" style="display:none"><path d="M17.94 17.94A10.07 10.07 0 0 1 12 20c-7 0-11-8-11-8a18.45 18.45 0 0 1 5.06-5.94M9.9 4.24A9.12 9.12 0 0 1 12 4c7 0 11 8 11 8a18.5 18.5 0 0 1-2.16 3.19m-6.72-1.07a3 3 0 1 1-4.24-4.24"/><line x1="1" y1="1" x2="23" y2="23"/></svg>
              </button>
            </div>
          </div>
      ...
      <div class="page-footer">Credentials saved to NVS when connected &bull; AP always stays active</div>
    </div>

    <!-- AUTONOMY PAGE -->
    <div class="page" id="page-autonomy">
      <div class="card">
        <div class="card-header">
          Behavioural Brain
          <button id="btn-autonomy-toggle" onclick="toggleAutonomy()">—</button>
        </div>
        <div class="card-body">
          <div class="status-row"><span class="label">State</span><span class="value" id="auto-state">—</span></div>
          <div class="status-row"><span class="label">Emotion</span><span class="value" id="auto-emotion">—</span></div>
          <div class="status-row"><span class="label">Sonar</span><span class="value" id="auto-sonar">—</span></div>
          <div class="status-row"><span class="label">Heading</span><span class="value" id="auto-heading">—</span></div>
          <div class="status-row"><span class="label">Interest</span><span class="value" id="auto-interest">—</span></div>
          <div class="status-row"><span class="label">Object</span><span class="value" id="auto-object">—</span></div>
          <div class="status-row"><span class="label">GPS</span><span class="value dim" id="auto-gps">—</span></div>
          <div class="status-row"><span class="label">Return Home</span><span class="value" id="auto-rth">—</span></div>
          <button onclick="setHome()" style="margin-top:8px">Set current position as HOME</button>
        </div>
      </div>
      <div class="page-footer">Polls /api/autonomy every 250ms &bull; Joystick overrides always</div>
    </div>
...
function switchTab(name) {
  ...
  if (name === 'connections') {
    ...
    if (list && list.querySelector('.scan-placeholder') && !list.querySelector('.net-item')) doScan();
  }
  if (name === 'settings') refreshSettings();
  if (name === 'autonomy') startAutonomyPoll();
  else if (autonomyPollTimer) { clearInterval(autonomyPollTimer); autonomyPollTimer = null; }
}
...
async function fetchAutonomy() {
  try {
    const d = await fetch(BASE + '/api/autonomy').then(r => r.json());
    const id = x => document.getElementById(x);
    if (id('auto-state')) id('auto-state').textContent = d.state || '—';
    if (id('auto-emotion')) id('auto-emotion').textContent = d.emotion || '—';
    if (id('auto-sonar')) id('auto-sonar').textContent = (d.sonar != null) ? d.sonar + ' cm' : '—';
    if (id('auto-heading')) id('auto-heading').textContent = (d.heading != null) ? d.heading + '\u00B0' : '—';
    if (id('auto-interest')) id('auto-interest').textContent = (d.interest != null) ? d.interest + '%' : '—';
    if (id('auto-object')) id('auto-object').textContent = d.objectDetected ? 'Yes' : 'No';
    if (id('auto-gps')) id('auto-gps').textContent = d.gpsValid ? (d.lat + ', ' + d.lon) : 'No fix';
    if (id('auto-rth')) id('auto-rth').textContent = d.rthActive ? (d.rthState + ' ' + d.rthDistance + 'm') : 'Idle';
    const btn = id('btn-autonomy-toggle');
    if (btn) btn.textContent = d.enabled ? 'Disable' : 'Enable';
  } catch (_) {}
}
function startAutonomyPoll() {
  if (autonomyPollTimer) clearInterval(autonomyPollTimer);
  fetchAutonomy();
  autonomyPollTimer = setInterval(fetchAutonomy, 250);
}
async function toggleAutonomy() {
  try {
    const d = await fetch(BASE + '/api/autonomy').then(r => r.json());
    await fetch(BASE + '/api/autonomy/enable?enable=' + (d.enabled ? '0' : '1'));
    fetchAutonomy();
  } catch (_) {}
}
async function setHome() {
  try {
    await fetch(BASE + '/api/autonomy/set_home');
    fetchAutonomy();
  } catch (_) {}
}
...
async function doScan() {
  ...
  try {
    ...
    list.innerHTML = sorted.map(n => {
      const quality = rssiToQuality(n.rssi || -90);
      const lock = (n.secure !== false) ? '<span class="lock-icon"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="3" y="11" width="18" height="11" rx="2"/><path d="M7 11V7a5 5 0 0 1 10 0v4"/></svg></span>' : '';
      const rssiStr = (n.rssi != null) ? '<span class="net-rssi">' + n.rssi + ' dBm</span>' : '';
      return '<div class="net-item" data-ssid="' + (n.ssid || '').replace(/&/g,'&amp;').replace(/"/g, '&quot;').replace(/</g,'&lt;') + '" onclick="selectNetFromEl(this)">' +
        '<span class="net-name">' + (n.ssid || '').replace(/</g,'&lt;') + '</span>' +
        '<span class="net-meta">' + rssiStr + lock +
        '<div class="signal-bars">' + signalBars(quality) + '</div></span></div>';
    }).join('');
  } catch(e) {
...
function selectNetFromEl(el) {
  const ssid = el.dataset.ssid || el.querySelector('.net-name')?.textContent || '';
  document.getElementById('inp-ssid').value = ssid;
  document.querySelectorAll('.net-item').forEach(n => n.classList.toggle('selected', n === el));
  document.getElementById('inp-pass').focus();
}
function selectNet(ssid) {
  document.getElementById('inp-ssid').value = ssid;
  document.querySelectorAll('.net-item').forEach(el =>
    el.classList.toggle('selected', (el.dataset.ssid || el.querySelector('.net-name')?.textContent) === ssid));
  document.getElementById('inp-pass').focus();
}
...
async function doClearCredentials() {
  ...
  btn.disabled = true;
  try {
    await fetch(BASE + '/wifi/clear');
    document.getElementById('connect-feedback').className = 'ok';
    document.getElementById('connect-feedback').textContent = 'Saved credentials cleared';
    document.getElementById('inp-ssid').value = '';
    document.getElementById('inp-pass').value = '';
    setTimeout(() => { document.getElementById('connect-feedback').textContent = ''; refreshStatus(); btn.disabled = false; }, 2000);
...
function togglePassVisible() {
  ...
  eye.style.display = vis ? '' : 'none';
  eyeOff.style.display = vis ? 'none' : '';
}
