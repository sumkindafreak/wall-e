...
bool sonarInit() {
  ...
  return true;  // Sonar always succeeds (GPIO only)
}
...
bool sonarIsObjectDetected(float maxRangeCm) {
  if (!s_valid) return false;
  return (s_distanceCm >= SONAR_MIN_CM && s_distanceCm <= maxRangeCm);
}

bool sonarIsObjectClose(float thresholdCm) {
  if (!s_valid) return false;
  return (s_distanceCm >= SONAR_MIN_CM && s_distanceCm < thresholdCm);
}
